...
#'
#' @param rctSample Data frame of RCT data. Must have columns `subgroup`, the
#'   treatment column named by `treatment`, and the outcome column named by `outcome`.
#' @param obsSample Data frame of observational data. Must have columns `subgroup`,
#'   the treatment column named by `treatment`, `propScore`, and the outcome column
#'   named by `outcome`.
#' @param outcome Character string. Name of the binary outcome column.
#' @param treatment Character string. Name of the binary treatment column (1 = treated,
#'   0 = control). Default `"Test"`.
...
#' @export
getParamEstimates <- function(rctSample, obsSample, outcome, treatment = "Test",
                              propScoreAdjust = TRUE) {
  ...
  rctStrataResults <- ddply(rctSample, .(subgroup), .fun = function(d) {
    ...
    n1 <- sum(trt)
    n0 <- sum(1 - trt)
    p1 <- ifelse((mu1 <- mean(d[[outcome]][trt == 1])) == 0,
                 0.5/n1, ifelse(mu1 == 1, (n1 - 0.5)/n1, mu1))
    p0 <- ifelse((mu0 <- mean(d[[outcome]][trt == 0])) == 0,
  ...
  obsStrataResults <- ddply(obsSample, .(subgroup), .fun = function(d) {
    ...
    trt <- d[[treatment]]
    if(propScoreAdjust) {
      p1 <- sum(d[[outcome]][trt == 1]/d$propScore[trt == 1])/
        sum(1/d$propScore[trt == 1])
      p0 <- sum(d[[outcome]][trt == 0]/(1-d$propScore[trt == 0]))/
        sum(1/(1 - d$propScore[trt == 0]))

      obsEst <- p1 - p0

      wts.trt <- 1/d$propScore[trt == 1]/sum(1/d$propScore[trt == 1])
      wts.ctl <-  1/(1-d$propScore[trt == 0])/sum(1/(1-d$propScore[trt == 0]))

      obsVar <- (sum(wts.trt^2*p1*(1-p1)) + sum(wts.ctl^2*p0*(1-p0)))

    } else {
      p1 <- mean(d[[outcome]][trt == 1])
      p0 <- mean(d[[outcome]][trt == 0])

      obsEst <- p1 - p0
      obsVar <- p1*(1-p1)/sum(trt == 1) + p0*(1-p0)/sum(trt == 0)
    }
...
#' @param treatment Character string. Name of the binary treatment column
#'   (1 = treated, 0 = control). Default `"Test"`.
